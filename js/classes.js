class DuckMover {
  ...
  liftoff() {
      ...
      let liftoffPosition = {x: newPosition.x, y: '750'};
      ...
      // Save this new position ready for the nextPosition call.
      this.currentPosition = liftoffPosition;
  }

  moveOnce() {
    // Pick a new spot on the page
    let nextPosition = this.generateNewPosition();
    
    // How far do we have to move?
    let distance = this.calcDistance(this.currentPosition, nextPosition);

    // Speed of this transition, rounded to 2DP
    let speed = Math.round((distance / this.pixels_per_second) * 100) / 100;
    this.redSquare.style.transition = `transform ${speed}s linear`;
    this.redSquare.style.transform = `translate(${nextPosition.x}px, ${nextPosition.y}px)`;

    // creates classes for animation directions
    while (true) {
      // Calculate distances  
      let xDistance = Math.round(Math.sqrt((this.currentPosition.x - nextPosition.x) * (this.currentPosition.x - nextPosition.x)));
      let yDistance = Math.round(Math.sqrt((this.currentPosition.y - nextPosition.y) * (this.currentPosition.y - nextPosition.y)));
      let duckDirection = document.querySelector(`#${this.redSquare.id}`).firstChild;

      if (this.currentPosition.x < nextPosition.x) {
        duckDirection.className = 'duck-right';

        if (this.currentPosition.y > nextPosition.y && xDistance < yDistance) {
          this.duckWay = 'up_';
            break;
        } else {
          this.duckWay = '';
  ...
  flyAway() {
    ...
    nextPosition = {x: nextPosition.x, y: -100}
    ...
    let speed = Math.round((distance / this.pixels_per_second) * 100) / 100;
    this.redSquare.style.transition = `transform ${speed}s linear`;
    this.redSquare.style.transform = `translate(${nextPosition.x}px, -100px)`;
